...
const Main = () => {
  ...
  const keys = ['userName', 'phoneNumber', 'email', 'orgName'];
  ...
  const search = ({datax}:props) => {
    return datax?.filter((item:UserObj) =>
    item.orgName.toLowerCase().includes(userContext?.query!) || item.userName.toLowerCase().includes(userContext?.query!) || item.phoneNumber.toLowerCase().includes(userContext?.query!) || item.email.toLowerCase().includes(userContext?.query!))
